  ...
  return (
    <StyledInput
      ...
      onBlur={() => {
        ...
        if (isNaN(parsed)) setLocalValue(String(value));
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <SettingSection title="Position & Size">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label="Width">
              <NumberInput
                value={widget.width}
                onChange={(v) => onUpdateWidget({ width: v })}
                min={10}
              />
            </Field>
            <Field label="Height">
              <NumberInput
                value={widget.height}
                onChange={(v) => onUpdateWidget({ height: v })}
                min={10}
              />
            </Field>
          </div>
                              ...
                              <>
                                <Field label="Font Size">
                                  <NumberInput
                                    ...
                                    onChange={(v) => updateCellOverride(selectedGridCell, 'fontSize', v)}
                                    min={8}
                                    max={200}
                                  />
                                </Field>
        ...
        <SettingSection title="Layer" defaultExpanded={false}>
          <Field label="Z-Index">
            <NumberInput
              value={widget.zIndex}
              onChange={(v) => onUpdateWidget({ zIndex: v })}
              min={0}
              max={100}
            />
          </Field>
  ...
  return (
    <div className="space-y-3">
      ...
      <Field label="Location">
        <div className="flex gap-1">
          ...
          <button
            ...
            className="flex-shrink-0 p-2 rounded-lg border border-border-light hover:bg-accent-light transition-colors"
      ...
      <LocationPickerModal
        ...
        initialLatitude={(widget.config.latitude as number) || 52.23}
        initialLongitude={(widget.config.longitude as number) || 21.01}
        initialLocation={(widget.config.location as string) || ''}
      />
