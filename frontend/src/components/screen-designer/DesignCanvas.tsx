...
export const DesignCanvas = forwardRef<DesignCanvasHandle, DesignCanvasProps>(function DesignCanvas({
  ...
  const MAX_ZOOM_MULTIPLIER = 5; // 5x beyond fit = pixel-level detail
  ...
  const calcFitScale = useCallback(() => {
    ...
    const availH = container.clientHeight - padding - 32;
    return Math.min(availW / width, availH / height);
  }, [width, height]);

  // Set initial fit scale on mount
  useEffect(() => {
  ...
  // Recalculate on window resize (not container resize, to avoid feedback loop from zooming)
  useEffect(() => {
    const handleResize = () => {
      const fit = calcFitScale();
      if (fit == null) return;
      setFitScale(fit);
      setScale(s => s > fit ? fit : s);
    };
  ...
  const zoomIn = useCallback(() => {
    setScale(s => Math.min(maxScale, Math.round(s * 1.25 * 100) / 100));
  }, [maxScale]);

  const zoomOut = useCallback(() => {
    setScale(s => Math.max(fitScale, Math.round((s / 1.25) * 100) / 100));
  }, [fitScale]);
  ...
  // Display percentage relative to fit scale (fit = 100%)
  const zoomPercent = useMemo(() => Math.round((scale / fitScale) * 100), [scale, fitScale]);

  // Ctrl+scroll zoom (needs non-passive listener for preventDefault)
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container) return;
    ...
    const handleWheel = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        setScale(s => {
          ...
          return Math.min(maxScale, Math.max(fitScale, Math.round(s * delta * 100) / 100));
        });
      }
    };
  ...
  // Undo/Redo history (capped to limit memory — each entry is ~1.5 MB)
  ...
  const handleBucketFill = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Save to history (capped at MAX_HISTORY entries)
    const newImageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    setHistory(prev => {
      const trimmed = [...prev.slice(0, historyIndex + 1), newImageData];
    ...
    if (isDrawing) {
      ...
      if (canvas) {
        ...
        if (ctx) {
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
          setHistory(prev => {
            const trimmed = [...prev.slice(0, historyIndex + 1), imageData];
            if (trimmed.length > MAX_HISTORY) {
              const overflow = trimmed.length - MAX_HISTORY;
              return trimmed.slice(overflow);
            }
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg-muted overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`p-1 rounded ${scale <= fitScale ? 'text-text-placeholder cursor-not-allowed' : 'text-text-muted hover:bg-bg-muted'}`}
              title="Zoom out"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              onClick={zoomFit}
              className="px-1.5 py-0.5 text-xs text-text-muted hover:bg-bg-muted rounded min-w-[3rem] text-center"
              title="Fit to view"
            >
              {zoomPercent}%
            </button>
            <button
              ...
              className={`p-1 rounded ${scale >= maxScale ? 'text-text-placeholder cursor-not-allowed' : 'text-text-muted hover:bg-bg-muted'}`}
              title="Zoom in"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      >
        <div style={{ width: width * scale + 6, height: height * scale + 38, margin: '0 auto', position: 'relative' }}>
          {/* Scaled canvas — transform doesn't affect layout, so wrapper has explicit scaled size */}
          <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left', position: 'absolute', top: 0, left: 0 }}>
            <div className="relative" style={{ paddingTop: 32 }}> {/* space for labels above canvas */}
