...
export function ScreenDesigner() {
  ...
  const backUrl = rawBack.startsWith('/') && !rawBack.startsWith('//') ? rawBack : '/screens';
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      ...
                      if (!isNaN(v) && v < 50) setCustomWidth('50');
                    }}
                    min={50}
                    max={2000}
                    placeholder="800"
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      const v = parseInt(customHeight);
                      if (!isNaN(v) && v < 50) setCustomHeight('50');
                    }}
                    min={50}
                    max={2000}
                    placeholder="480"
                ...
                <button
                  onClick={() => {
                    const w = Math.max(50, Math.min(2000, parseInt(customWidth) || 800));
                    const h = Math.max(50, Math.min(2000, parseInt(customHeight) || 480));
                    setCustomWidth(String(w));
                    setCustomHeight(String(h));
                    handleSelectResolution(w, h);
                  }}
                  disabled={!customWidth && !customHeight}
                  className="mt-5 px-4 py-2 text-sm font-medium text-text-inverse bg-accent rounded-lg hover:bg-accent-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Use
                </button>
              </div>
