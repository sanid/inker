...
export async function validateUrlSafety(url: string, options?: UrlSafetyOptions): Promise<void> {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  // Always block internal service hostnames regardless of settings
  if (BLOCKED_HOSTNAMES.includes(hostname)) {
    throw new Error('URLs pointing to internal services are not allowed');
  }

  // Skip private IP checks when local network is allowed
 ...
 */
export function createSafeHttpAgent(options?: UrlSafetyOptions): http.Agent {
  if (options?.allowLocalNetwork) {
 ...
 */
export function createSafeHttpsAgent(options?: UrlSafetyOptions): https.Agent {
  if (options?.allowLocalNetwork) {
